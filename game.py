...
class Juego:
    def __init__(self):
        ...
        self.display = pygame.Surface(self.screen_display, pygame.SRCALPHA)
        self.display_2 = pygame.Surface(self.screen_display)
        ...
        self.zoom = 1.0  # El zoom inicial es 1.0 (sin zoom)
        self.base_resolution = (320, 240)
    ...
    def run(self):
        while True:
            ...
            while True:
                ...
                render_scroll = (
                    int((self.player.rect().centerx - self.screen.get_width() / 2) * self.zoom),
                    int((self.player.rect().centery - self.screen.get_height() / 2) * self.zoom)
                )
                ...
                if not self.dead:
                    self.player.update(self.tilemap, ((self.movement[1] - self.movement[0]) / self.zoom, 0))
                    self.player.render(self.display, offset=(int(render_scroll[0] / self.zoom), int(render_scroll[1] / self.zoom)))
                ...
                for enemy in self.enemies.copy():
                    ...
                    enemy.render(self.display, offset=(int(render_scroll[0] / self.zoom), int(render_scroll[1] / self.zoom)))
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_PLUS:
                            self.zoom *= 1.1  # Aumentar el zoom en un 10%
                        elif event.key == pygame.K_MINUS:
                            self.zoom /= 1.1  # Disminuir el zoom en un 10%
                ...
                self.screen.blit(pygame.transform.scale(self.prueba8, self.screen.get_size()), screenshake_offset)
                scaled_display = pygame.transform.scale(self.prueba8, self.current_resolution)
    ...
    def draw_death_count(self):
        ...
        death_text = font.render(f'Muertes: {self.death_count}', True, (255, 255, 255))
        
        # Adjust the position just above the player, considering the scroll offset
        player_rect = self.player.rect()
        text_position = (
            player_rect.centerx - death_text.get_width() // 2 - self.scroll[0],
            player_rect.top - 10 - self.scroll[1]
        )
        
        self.screen.blit(death_text, text_position)
    from entities import Player, Enemy
from utils import load_image, load_images, Animation
from tilemap import Tilemap
from clouds import Clouds
...
class Juego:
    ...
    def handle_resize_event(self, event):
        if event.type == pygame.VIDEORESIZE:
            ...
            scaled_resolution = (int(self.base_resolution[0] * self.zoom), int(self.base_resolution[1] * self.zoom))
            self.display = pygame.Surface(self.current_resolution, pygame.SRCALPHA)
            self.display_2 = pygame.Surface(self.current_resolution)
            

            # Escalar la imagen de fondo solo cuando se redimensiona la pantalla
            self.assets['background'] = pygame.transform.scale(self.assets['background'], (int(self.current_resolution[0] * self.zoom), int(self.current_resolution[1] * self.zoom)))
